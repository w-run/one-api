...
<p align="center">
  <a href="https://github.com/w-run/one-api#部署">部署教程</a>
  ·
  <a href="https://github.com/w-run/one-api#使用方法">使用方法</a>
  ·
  <a href="https://github.com/w-run/one-api/issues">意见反馈</a>
  ·
  <a href="https://github.com/w-run/one-api#截图展示">截图展示</a>
  ·
  <a href="https://openai.example.com/">在线演示</a>
  ·
  <a href="https://github.com/w-run/one-api#常见问题">常见问题</a>
  ·
  <a href="https://github.com/w-run/one-api#相关项目">相关项目</a>
  ·
...
> [!NOTE]
> 稳定版 / 预览版镜像地址：[demo629/one-api](https://hub.docker.com/repository/docker/demo629/one-api)
> 或者 [ghcr.io/w-run/one-api](https://github.com/w-run/one-api/pkgs/container/one-api)
>
> alpha 版镜像地址：[demo629/one-api-alpha](https://hub.docker.com/repository/docker/demo629/one-api-alpha)
> 或者 [ghcr.io/w-run/one-api-alpha](https://github.com/w-run/one-api/pkgs/container/one-api-alpha)
...
# 例如：
docker run --name one-api -d --restart always -p 3000:3000 -e SQL_DSN="root:123456@tcp(localhost:3306)/oneapi" -e TZ=Asia/Shanghai -v /home/ubuntu/data/one-api:/data demo629/one-api
```
...
如果启动失败，请添加 `--privileged=true`，具体参考 https://github.com/w-run/one-api/issues/482 。

如果上面的镜像无法拉取，可以尝试使用 GitHub 的 Docker 镜像，将上面的 `demo629/one-api` 替换为 `ghcr.io/w-run/one-api` 即可。
...
```
server{
   server_name openai.example.com;  # 请根据实际情况修改你的域名
...
### 手动部署
1. 从 [GitHub Releases](https://github.com/w-run/one-api/releases/latest) 下载可执行文件或者从源码编译：
   ```shell
...
详见 [#175](https://github.com/w-run/one-api/issues/175)。

如果部署后访问出现空白页面，详见 [#97](https://github.com/w-run/one-api/issues/97)。
...
注意修改端口号，之后在页面上设置接口地址（例如：https://openai.example.com/ ）和 API Key 即可。

#### ChatGPT Web
项目主页：https://github.com/Chanzhaoyu/chatgpt-web

```bash
docker run --name chatgpt-web -d -p 3002:3002 -e OPENAI_API_BASE_URL=https://openai.example.com -e OPENAI_API_KEY=sk-xxx chenzhaoyu94/chatgpt-web
```
...
你需要在各种用到 OpenAI API 的地方设置 API Base 为你的 One API 的部署地址，例如：`https://openai.example.com`，API Key 则为你在 One API 中生成的令牌。
...
5. `FRONTEND_BASE_URL`：设置之后将重定向页面请求到指定的地址，仅限从服务器设置。
   + 例子：`FRONTEND_BASE_URL=https://openai.example.com`
