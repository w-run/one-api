...
const EditModal = ({ open, channelId, onCancel, onOk }) => {
  ...
  // 自动生成模型映射关系
  const generateModelMapping = async (models, setField) => {
    ...
    const shortNames = {};
    const shortNameCount = {};

    // 第一轮：生成短名并统计重复
    models.forEach((model) => {
      const id = typeof model === 'string' ? model : model.id;
      let short = generateShortName(id);
      if (!shortNameCount[short]) {
        shortNameCount[short] = 0;
      }
      shortNameCount[short]++;
      shortNames[id] = short;
    });
    ...
    models.forEach((model) => {
      const id = typeof model === 'string' ? model : model.id;
      let short = shortNames[id];
      if (shortNameCount[short] > 1 && id.includes('/')) {
        const provider = id.split('/')[0].toLowerCase();
        short = short + '-' + provider;
      }
      finalNames[id] = short;
    });
    ...
    // 构建映射 JSON
    const mapping = {};
    const mappedModelList = [];
    models.forEach((model) => {
      const id = typeof model === 'string' ? model : model.id;
      const short = finalNames[id];
      mapping[short] = id;
      mappedModelList.push({ id: short, group: '映射模型' });
    });

    // 填充模型列表和映射关系
    setField('models', mappedModelList);
    setField('model_mapping', JSON.stringify(mapping, null, 2));
    showSuccess(`已生成 ${mappedModelList.length} 个模型的映射关系`);
  };
            ...
            <form noValidate onSubmit={handleSubmit}>
              ...
              >
                <ButtonGroup variant="outlined" aria-label="small outlined primary button group">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        // 如果密钥为空且有渠道ID，从数据库回退读取
                        if (!values.key && channelId) {
                          req.channel_id = channelId;
                        }
                        const res = await API.post('/api/channel/fetch_models', req);
                        const { success, message, data } = res.data;
                        if (success) {
                          const models = data.map((id) => ({ id, group: '远程获取' }));
                  ...
                  <Button
                    onClick={() => {
                      generateModelMapping(values.models, setFieldValue);
                    }}
                  >
                    生成模型映射
                  </Button>
